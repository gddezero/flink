...
public final class FactoryUtil {
    ...
    public static DynamicTableSource createTableSource(
        ...
        final DefaultDynamicTableContext context =
                new DefaultDynamicTableContext(
                        objectIdentifier, catalogTable, configuration, classLoader);
        try {
     ...
     * }</pre>
     *
     * <p>Note: The format option parameter of {@code helper.discoverScanFormat(formatFactoryClass,
     * formatOption)} and {@code helper.discoverSinkFormat(formatFactoryClass, formatOption)} must
     * be 'format' or with '.format' suffix (e.g. {@link #FORMAT}, {@link #KEY_FORMAT} and {@link
     * #VALUE_FORMAT}). The discovery logic will replace 'format' with the factory identifier value
     * as the format prefix. For example, assuming the identifier is 'json', if format option key is
     * 'format', then format prefix is 'json.'. If format option key is 'value.format', then format
     * prefix is 'value.json'. The format prefix is used to project the options for the format
     * factory.
     *
     ...
     * CatalogTable, ReadableConfig, ClassLoader)}, and {@link #createTableSink(Catalog,
     * ObjectIdentifier, CatalogTable, ReadableConfig, ClassLoader)} are not applicable.
     */
    ...
    public static void validateFactoryOptions(Factory factory, ReadableConfig options) {
        ...
        factory.optionalOptions().forEach(option -> readOption(options, option));
    }
